...
:: 1. Check for Certificate Password
if "%CSC_KEY_PASSWORD%"=="" (
...
if "!SIGNTOOL_PATH!"=="" (
    ...
    echo To enable signing, install the Windows SDK.
) else (
...
echo.
echo [2/2] Building Frontend (local only, no publish)...
cd frontend
...
:: Get version for output path reference
for /f "tokens=*" %%i in ('node -p "require('./package.json').version"') do set "VERSION=%%i"

:: Build locally — no publish
echo [INFO] Running Electron-Builder (local build only)...
call npm run build -- --publish never
if %errorlevel% neq 0 (
    echo.
    echo [ERROR] Build failed!
    echo Check for signing errors or missing dependencies above.
...
if exist "!INSTALLER_PATH!" (
    ...
    echo.
    echo [INFO] Installer located at:
    echo        frontend\!INSTALLER_PATH!
) else (
    echo [WARNING] Installer not found at expected path: !INSTALLER_PATH!
    echo Check the frontend\dist\ folder manually.
)
...
echo ==========================================
echo.
echo   Installer: frontend\dist\Windows-Runtime-Host-Setup-!VERSION!.exe
echo   No files were uploaded to GitHub.
echo.
pause
